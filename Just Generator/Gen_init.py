...
line_colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'k']
markers = ['x', 'o', '+', 'D', 'v', '*']

initmethods = [torch.nn.init.xavier_uniform_, torch.nn.init.xavier_normal_, torch.nn.init.kaiming_normal_, torch.nn.init.orthogonal_, "average", "gained"]
initnames = ["Xavier $\mathcal{U}$", "Xavier $\mathcal{N}$", "He $\mathcal{N}$", "Ortho", "Average", "Gain adjusted"]
...
for i, method in enumerate(initmethods):
    layerOptions = {'randnormweights':True, 'normalize':False}
    if method == "gained":
        method = torch.nn.init.xavier_normal_
        layerOptions["biasgain"] = 0.005
        layerOptions["weightgain"] = 0.4
    else:
        layerOptions["biasgain"] = 1.0
        layerOptions["weightgain"] = 1.0
    layerOptions["initmethod"] = method

    model = Generator_seq(layer, N, rank, HR_dim, HR_dim, 4, layerOptions, generatorOptions)
    epoch_loss, info = train(model)

    ax[0].plot(list(range(1,num_epochs+1)), epoch_loss, c=line_colors[i])
    ax[0].scatter(list(range(1,num_epochs+1,4)), epoch_loss[::4], c=line_colors[i], marker=markers[i], s = 40, label=initnames[i])
    ax[0].scatter(num_epochs, epoch_loss[-1], c=line_colors[i], marker=markers[i])
ax[0].set_title("CP with SL1 Loss, $N$ = {}, $r$ = {}".format(N, rank), fontsize=fs)
ax[0].set_yscale('log')
ax[0].grid(True)
ax[0].set_ylim([1.e-3,100])
...
for i, method in enumerate(initmethods):
    ...
    ax[1].plot(list(range(1,num_epochs+1)), epoch_loss, c=line_colors[i])
    ax[1].scatter(list(range(1,num_epochs+1,4)), epoch_loss[::4], c=line_colors[i], marker=markers[i], label=initnames[i])
    ax[1].scatter(num_epochs, epoch_loss[-1], c=line_colors[i], marker=markers[i])
ax[1].set_title("TT with SL1 Loss", fontsize=fs)
ax[1].set_yscale('log')
ax[1].grid(True)
ax[1].set_ylim([1.e-3,100])
ax[1].set_xlabel('Epochs', fontsize=fs)
ax[1].legend(prop={'size': 14})
ax[1].tick_params(labelleft=False)
fig.subplots_adjust(wspace=0.02, hspace=0.01)

filename = "Just Generator/initmethods" + timestamp + ".png"
