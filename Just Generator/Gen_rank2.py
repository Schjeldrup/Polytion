...
class Autoencoder_seq(torch.nn.Module):
    def __init__(self, layer, N, rank, bottleneck_dim, HR_dim, downscalefactor, scalefactor, layerOptions, generatorOptions):
        ...
        self.encoder = Generator_seq(layer, N, rank, bottleneck_dim, bottleneck_dim, downscalefactor, layerOptions, generatorOptions)
        self.decoder = Generator_seq(layer, N, rank, HR_dim, HR_dim, scalefactor, layerOptions, generatorOptions)
...
def getError(images, targets, model):
    output = []
    for image in images:
        test = torch.tensor(image).reshape(1,1,LR_dim,LR_dim)
        output.append(model(test).reshape(HR_dim,HR_dim))
    return sum([lossfunc(torch.tensor(i).to(device), j.clone()) for i,j in zip(targets, output)])/len(images)
...
ax[0].set_title("SL1 loss, $N$ = {}".format(N), fontsize=fs)
ax[0].set_yscale('log')
ax[0].set_ylim([1.0e-3, 2])
...
for i, rank in enumerate([5, 50, 100]):
    model = Generator_seq(layer, N, rank, HR_dim, HR_dim, 4, layerOptions, generatorOptions)
    #model = Autoencoder_seq(layer, N, rank, bottleneck_dim, HR_dim, downscalefactor, scalefactor, layerOptions, generatorOptions)

    epoch_loss, info = train(model)

    ax[0].plot(list(range(1,num_epochs+1)), epoch_loss, c=colors[i], ls=lines[i], label='$r$ = {}'.format(rank))
    ax[0].scatter(list(range(1,num_epochs+1,30)), epoch_loss[::30], c=colors[i], marker=markers[i])
    ...
    trainset_error = getError(train_LRimages, train_HRimages, model)
    testset_error = getError(test_LRimages, test_HRimages, model)

    ax[i+2].imshow(trainimage, cmap='gray')
    ax[i+2].set_title('$r$ = {}'.format(rank), fontsize=fs)
    ax[i+2].axes.xaxis.set_ticks([])#set_xticklabels([])
    ax[i+2].axes.yaxis.set_ticks([])#set_yticklabels([])
    ax[i+2].set_xlabel('train = {:.2e}   test = {:.2e} \n PSNR = {:.2f}'.format(trainset_error, testset_error, psnrscore), fontsize=15)

ax[0].legend(prop={'size': 14})
fig.subplots_adjust(wspace=0.01, hspace=0.01)

timestamp = time.strftime("%d-%m-%Y_%H:%M:%S")
filename = "Just Generator/AErankplot" + timestamp + ".png"
