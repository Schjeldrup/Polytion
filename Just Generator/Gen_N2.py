...
def getError(images, model):
    ...
    for image in images:
        ...
        if cuda:
            ...
            output.append(model(test).reshape(HR_dim,HR_dim).detach().numpy())
    return sum([lossfunc(i,j) for i,j in zip(images, output)])/len(images)
...
# averagepath = imagefolderpath + '/average'
...
for i, N in enumerate([0, 1, 2]):
    ...
    ax[i+2].set_title('$N$ = {}'.format(N), fontsize=fs)
...
filename = "Just Generator/Nplot" + timestamp + ".png"
